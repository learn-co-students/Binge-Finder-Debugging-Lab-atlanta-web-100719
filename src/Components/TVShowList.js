...
class TVShowList extends Component {
  ...
  mapAllShows = () => {
    ...
    if (!!this.props.searchTerm && !!this.props.filterRating) {
      return this.props.shows.map((s) => {
        if (s.name.toLowerCase().includes(this.props.searchTerm) && s.rating.average >= this.props.filterRating) {
      ...
      return this.props.shows.map((s) => {
        if (s.rating.average >= this.props.filterRating) {
          return (<TVShow show={s} key={s.id} selectShow={this.props.selectShow} />)
        } else {
          return null
        }
